// src/components/review/AudioContentPanel.tsx
'use client';
...
export function AudioContentPanel({ content }: AudioContentPanelProps) {
  ...
  const getAudioUrl = () => {
    ...
    if (content.audio_filename) {
      ...
      const packageId = packageIdMatch ? packageIdMatch[0] : 'unknown';
      
      // Use your backend API endpoint
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000';
      return `${apiUrl}/api/v1/audio/${packageId}/${content.audio_filename}`;
    }
    ...
    if (content.audio_file_path) {
      ...
      const packageId = packageIdMatch ? packageIdMatch[0] : 'unknown';
      return `${apiUrl}/api/v1/audio/${packageId}/${filename}`;
    }
  ...
  const handleRegenerate = () => {
    if (confirm('Are you sure you want to regenerate the audio? This will replace the current audio content.')) {
